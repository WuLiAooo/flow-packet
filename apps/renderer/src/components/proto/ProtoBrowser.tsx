...
type BrowserKind = 'request' | 'wait_response'
type BrowserFile = FileInfo & { Messages: MessageInfo[] }
...
export function ProtoBrowser() {
  ...
  const [browserKind, setBrowserKind] = useState<BrowserKind>('request')
  ...
  const predicate = browserKind === 'request' ? isCgMessage : isGcMessage

  const messageFiles = useMemo(() => {
    return files
      .map((file) => {
        const matchedMessages = (file.Messages ?? []).filter(predicate)
        if (matchedMessages.length === 0) return null
        return { ...file, Messages: matchedMessages }
      })
      .filter(Boolean) as BrowserFile[]
  }, [files, predicate])

  const filteredFiles = useMemo(() => {
    const q = deferredSearch.trim().toLowerCase()
    if (!q) return messageFiles
    ...
    return messageFiles
      ...
      .filter(Boolean) as BrowserFile[]
  }, [deferredSearch, messageFiles])

  const messageCount = useMemo(
    () => messageFiles.reduce((count, file) => count + file.Messages.length, 0),
    [messageFiles],
  )
  ...
  const rows = useMemo<RowItem[]>(() => {
    ...
    filteredFiles.forEach((file) => {
      ...
      nextRows.push({
        kind: 'file',
        key: `file:${browserKind}:${file.Path}`,
        file,
        open,
      })
      ...
      file.Messages.forEach((message) => {
        nextRows.push({
          kind: 'message',
          key: `message:${browserKind}:${message.Name}`,
          filePath: file.Path,
          message,
    ...
    return nextRows
  }, [browserKind, filteredFiles, isSearching, openFiles, routeMappingByRequest])
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden px-2.5">
      <div className="flex h-8 shrink-0 items-center justify-between px-2">
        <span className="text-xs font-medium text-muted-foreground">Protocol Browser</span>
      </div>
      ...
      <div className="mb-2 shrink-0 px-2">
        <div className="inline-flex rounded-md border border-input bg-background p-0.5 shadow-xs">
          <button
            type="button"
            className={cn(
            ...
            onClick={() => setBrowserKind('request')}
          >
            Cg
          </button>
          <button
            ...
            className={cn(
              'rounded px-2 py-1 text-xs transition-colors',
              browserKind === 'wait_response'
                ? 'bg-primary text-primary-foreground'
                : 'text-muted-foreground hover:text-foreground',
            )}
            onClick={() => setBrowserKind('wait_response')}
          >
            Gc
          </button>
        </div>
      ...
      <div className="mb-2 shrink-0 px-2 text-xs font-medium text-muted-foreground">
        {browserKind === 'request' ? 'Cg Messages' : 'Gc Messages'} {messageCount > 0 ? `(${messageCount})` : ''}
      </div>
              ...
              return (
                ...
                >
                  {row.kind === 'file' ? (
                    ...
                    <WaitResponseMessageRow message={row.message} />
                  )}
                </div>
              )
            })}
          ...
          <div className="px-3 py-4 text-center">
            <span className="text-xs text-muted-foreground">
              {messageFiles.length === 0
                ? browserKind === 'request'
                  ? 'No sendable Cg messages found.'
                  : 'No waitable Gc messages found.'
                : browserKind === 'request'
                  ? 'No matching Cg messages.'
                  : 'No matching Gc messages.'}
            </span>
          </div>
        )}
      </div>
...
function RequestMessageRow({
  ...
  const handleDragStart = (e: React.DragEvent) => {
    e.dataTransfer.setData('application/flow-packet-message', JSON.stringify({
      kind: 'request',
      message,
    }))
      ...
      {dialogOpen && (
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Configure Route Mapping</DialogTitle>
...
function WaitResponseMessageRow({ message }: { message: MessageInfo }) {
  const handleDragStart = (e: React.DragEvent) => {
    e.dataTransfer.setData('application/flow-packet-message', JSON.stringify({
      kind: 'wait_response',
      message,
  ...
  return (
    <button
      ...
      onDragStart={handleDragStart}
    >
      <Box className="size-4 shrink-0 text-emerald-600" />
      <span className="truncate text-xs">{message.ShortName}</span>
      {message.MessageID ? (
        <Badge
          variant="secondary"
